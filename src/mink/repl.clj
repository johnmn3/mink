...
(defn repl-id []
  (if-let [kv (first (filter #(= *out* (val %)) (:repl->out @state)))]
    (key kv)
    (let [repl (.getId (Thread/currentThread))]
...
(defn current-client []
  (let [client-id (get-in @state [:repl->client (repl-id)]) #_(:current-client-id @state)]
...
(defn current-ns []
  (or (get-in @state [:client->ns (current-client)])
      'cljs.user))
...
(defn list-clients []
  (let [clients (:clients @state)
    ...
    (->> clients
         (map (fn [[id _]]
                (str (if (= id current) "* " "- ")
                     (-> @state :client->ns (get id) (or 'cljs.user))
                     " => "
                     id)))
         (string/join "\n"))))
...
(defn repl-env*
  ...
  (merge (BrowserEnv.)
    {:host host
     ...
     :static-dir (cond-> ["." "out/"] output-dir (conj output-dir))
     :preloaded-libs '[mink.browser mink.preload cljs.repl clojure.browser.repl clojure.browser.repl.preload]
     :src "src/"
